...
from config import FINAM_API_URL, FINAM_TOKEN
...
def _get_jwt() -> str:
    ...
    try:
        ...
        logger.info("Finam /sessions status: %s", resp.status_code)
        if resp.status_code == 401:
            logger.error("Finam 401 Unauthorized — token may be invalid or expired")
            try:
                body = resp.text[:500]
                logger.error("Finam 401 response: %s", body)
            except:
                pass
            raise RuntimeError("Finam JWT auth failed (401). Check FINAM_TOKEN in .env")
...
def fetch_historical(
    ...
    symbol: e.g. "BMM6@RTSX" or "BMK6@RTSX"
    ...
    while current_to > from_ms:
        ...
        try:
            resp = httpx.get(
                f"{FINAM_API_URL}/instruments/{symbol}/bars",
                params=params,
            ...
            logger.warning("Finam bars HTTP error for %s %s: %s", symbol, timeframe, exc)
            break
        except Exception as exc:
            logger.warning("Finam bars request failed for %s %s: %s", symbol, timeframe, exc)
            break
...
def fetch_current(symbol: str) -> dict:
    ...
    try:
        resp = httpx.get(
            f"{FINAM_API_URL}/instruments/{symbol}/quotes/latest",
            headers=headers,
            timeout=_CLIENT_TIMEOUT,
            follow_redirects=True,
        )
    ...
    try:
        resp = httpx.get(
            f"{FINAM_API_URL}/instruments/{symbol}/orderbook",
            headers=headers,
            timeout=_CLIENT_TIMEOUT,
            follow_redirects=True,
        )
